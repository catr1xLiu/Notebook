...
\textbf{Solution: } According to definitions, we can re-write the statements to the following: \[
	\begin{cases}
		A \subseteq B & \iff (x \in B \implies x \in A) \\
		A \cap B = A & \iff ((x \in A \land x \in B) \iff (x \in A)) \\ 
		A \cup B = B & \iff ((x \in A \lor x \in B) \iff (x \in B)) \\
		A \cap \overline{B} = \emptyset & \iff ((x \in A \land \lnot (x \in B)) = (x \in \emptyset)) \\
		\overline{A} \cup B = \mathcal{U} & \iff (\lnot(x \in A) \or (x \in B)) = (x \in \mathcal{U}) \\
		\overline{B} \subseteq \overline{A} & \iff \lnot(x \in A) \implies \lnot(x \in B) \\
