...
% }
\title{\LARGE \bf
Modeling Aging Signatures in Human Motion via Motion Diffusion Models for Elder Care Robotics%
\thanks{This work was supported by the National Research Council Canada (NRC) through the Aging in Place Challenge Program.}
}
...
\author{
Yiran Liu$^{1,2}$, Eugene Zhao$^{1,2}$, Karim Habashy$^{1,2}$, Joanna Peng$^{1,2}$, Jason Peng$^{3}$, Chang Shu$^{1}$, Pengcheng Xi$^{1}$ \\
...
\begin{figure*}[!t]
    \centering
    \IfFileExists{sec/figs/23 - Future Work Architecture.png}%
    {\includegraphics[width=0.85\textwidth]{sec/figs/23 - Future Work Architecture.png}}%
    {\fbox{\parbox[c][2cm][c]{0.85\textwidth}{\centering Figure missing: \texttt{sec/figs/23 - Future Work Architecture.png}}}}
    \caption{Proposed continuous age-conditioning architecture.}
