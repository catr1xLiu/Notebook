...
\doctype{Week 10 Experiment Report}
\title{ST-GCN++ Age Classification Training Results \& Analysis}

\author{Yiran Liu}

\dates{Compiled on \today}
\footinfo{Week 10 Experiment Report}
\theday{\today}
...
This week, two remaining issues in the SMPL$\rightarrow$NTU-25 conversion pipeline were resolved, and the cleaned Van Criekinge dataset (450 clips, 138 subjects) was verified by running the pretrained action recognition model on the full dataset---achieving 100\% walking-action classification.

With the data pipeline validated, I made a few more attempts to fine-tune the ST-GCN++ model with a custom 3-class age classifier (Young $<$40, Adult 40--64, Elderly $\geq$65). The initial results were disappointing, but through a few iterations, the best model achieved 64.60\% validation accuracy (31.6\% above the random baseline). To help determine the best approach moving forward, I have written this detailed report for your review and for our upcoming discussion.
\normalfont
\end{abstract}
...
\subsection{Improvement1: Backbone Unfreezing Sweep}
...
\subsection{Improvement 2: Train/Val Split}

The observed overfitting is a dangerous sign indicating that the model could be memorizing the traits of specific subjects in the dataset, rather than learning generalized aging patterns. To rule out this source of bias, we must ensure that subjects are strictly mutually exclusive between the training and validation sets.

\textbf{Fix:} Round-robin subject assignment. Subjects are sorted numerically, and every 4th subject is assigned to the validation set:
...
This produces a $\sim$75/25 split (337 train / 113 val) ensuring that subjects from across the full age range are represented in the validation set. This was combined with a reduced batch size (16$\rightarrow$8) to introduce implicit regularization through gradient noise:
...
The new split method improved the 1-block validation accuracy by \textbf{+6.16pp} and stabilized the 2-block configuration's overfitting dynamics dramatically---the best epoch shifted from 4 $\rightarrow$ 30, providing a 30-epoch window for generalization instead of a narrow 4-epoch window.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\columnwidth]{figures/figure4_split_comparison_combined.png}
    \caption{Comparison of validation curves before and after the split improvement for the 2-block configuration. The Session 1 random split peaks at epoch 4 and then oscillates wildly between 42--58\%, while the Session 2 round-robin split climbs gradually to 64.60\% at epoch 30 and remains much more stable thereafter. The same Y-axis scale is used for direct comparison.}
...
\subsection{Dataset Size Constraint}

The primary bottleneck is dataset size: The Van Criekinge dataset only contains 450 clean clips from 138 subjects. For context, the NTU RGB+D pretraining dataset has 113K clips. Both the 718K trainable parameters in the 2-block configuration and the 363K params in the 1-block configuration exceed what this dataset can effectively constrain, leading to memorisation (98+\% train accuracy) within a few epochs. 

\subsection{Frozen Backbone Feature Invariance}

The frozen backbone experiment (43.68\% val, barely above random) shows that pre-trained STGCN++ backbone is largely \textbf{invariant to the action style} that age classification depends on. The model was pretrained on NTU-120 action dataset, and is purposed to extract \textit{what action} being performed (walking vs. sitting vs. throwing). Therotically, the model should learn to ignore \textit{how} the action is performed, and \textit{by whom} (young adults vs. elderly subjects). 

This means any backbone architecture (CTR-GCN, DG-STGCN, etc.) pretrained on action recognition may face the same limitation when frozen. Unfreezing is necessary, but (as mentioned above) it also causes overfitting.
...
The round-robin split and reduced batch size substantially extended the practical training window for the 2-block configuration, but the fundamental overfitting tendency remains. 

\section{Next Steps}

Fine-tuning the regularisation parameters (dropout, weight decay) and using data augmentation are likely the most promising next steps. The detailed plan should be discussed in our meeting.
